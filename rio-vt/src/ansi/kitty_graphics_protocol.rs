    ...
    match cmd.format {
        Format::Png => {
            ...
            let result = {
                ...
                // Bound decode allocation before pixels are materialized:
                // a small PNG can declare huge dimensions and OOM the
                // process (the MAX_DIMENSION check below only runs
                // post-decode).
                let img = {
                    use std::io::Cursor;
