...
pub fn parse(params: &[&[u8]]) -> Option<(GraphicData, u8)> {
    ...
    #[cfg(feature = "graphics")]
    {
        // Bound decode allocation: a small OSC 1337 payload can declare
        // huge dimensions (or use a multi-strip compressed TIFF) and OOM
        // the process during decode — the post-decode
        // MAX_GRAPHIC_DIMENSIONS clamp runs too late to stop it.
        // ImageReader.limits caps the allocation before pixels are
        // materialized.
